...
For a request that already has a spec or plan (any framework), read the actual
documents. Preserve terminology, requirements, and accepted decisions. Add a routing
and execution mapping, not a rival plan. This skill is self-contained. Use installed skill names as exposed by the host;
do not pretend an absent skill ran.
...
Fable owns threat analysis and core correctness, not merely a final glance.
...
can finish and Fable can independently accept or reject. Prefer one vertical,
...
every token; reserve implementation discretion for DeepSeek inside the contract.
